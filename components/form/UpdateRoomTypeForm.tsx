...
function UpdateRoomTypeForm({ id }: { id: number }) {
  ...
  useEffect(() => {
    async function getRoomType() {
      const res = await fetch(`${baseUrl}/api/room-type/${id}`);
      if (!res.ok) {
      ...
      setRoomType(data);
      form.reset({
        facilities: data.facilities.map((f: any) => f.facilityId),
        type: data.roomType,
        img: data.roomImg,
      });
      setPreview(data.roomImg);
      setOldImage(data.roomImg);
    }
    getRoomType();
  }, []);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    if (oldImage) {
      const updatedValues = { ...values, img: oldImage };
...
export default UpdateRoomTypeForm;
